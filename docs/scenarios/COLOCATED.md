...
资源较少且能够接受组件共享主机与故障域。每台主机运行 MySQL、Router、HAProxy 和 Keepalived。 需要 3 台目标主机，另备 Ansible 控制端。

```mermaid
flowchart LR
    N1[主机 1: MySQL + Router + LB] --- N2[主机 2: MySQL + Router + LB]
    N2 --- N3[主机 3: MySQL + Router + LB]
```

db1/db2/db3 同时加入数据库、Router 和 haproxy_lb 角色组。默认端口互不冲突；若自定义端口，应同时核对各组件占用。不能让不同别名反复配置同一个服务。

## 配置

完成 [公共准备](COMMON.md)，其中选择 `TOPOLOGY=colocated`。
模板可直接查看：[角色与主机定义](https://github.com/seaworld008/auto-install-mysql-innodb-cluster/blob/main/examples/topologies/colocated.yml)。
