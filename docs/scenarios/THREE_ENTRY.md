# 三节点接入层：3 MySQL + 3 Router + 3 入口

## 适用条件与架构

需要在接入层提供更多节点、分担连接或留出维护空间。 需要 9 台目标主机，另备 Ansible 控制端。

```mermaid
flowchart LR
    DB[MySQL × 3] --- R[独立 Router × 3]
    R --- L[独立 HAProxy + Keepalived × 3]
```

router1/router2/router3 处于同一 mysql_router 组，lb1/lb2/lb3 处于同一 haproxy_lb 组，优先级分别为 150/120/100。三个 LB 使用同一个 VIP，只有一个当前持有者。

## 配置

完成 [公共准备](COMMON.md)，其中选择 `TOPOLOGY=three-entry`。
模板可直接查看：[角色与主机定义](https://github.com/seaworld008/auto-install-mysql-innodb-cluster/blob/main/examples/topologies/three-entry.yml)。
