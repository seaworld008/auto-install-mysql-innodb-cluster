# 独立部署：3 MySQL + 2 Router + 2 入口

## 适用条件与架构

数据库、路由、入口各自使用主机，便于分别规划资源和维护窗口。 需要 7 台目标主机，另备 Ansible 控制端。

```mermaid
flowchart LR
    DB[MySQL × 3] --- R[独立 Router × 2]
    R --- L[独立 HAProxy + Keepalived × 2]
```

MySQL 的三个节点、Router 的两个节点和两个入口主机分别属于不同角色组。

## 配置

完成 [公共准备](COMMON.md)，其中选择 `TOPOLOGY=dedicated`。
模板可直接查看：[角色与主机定义](https://github.com/seaworld008/auto-install-mysql-innodb-cluster/blob/main/examples/topologies/dedicated.yml)。
