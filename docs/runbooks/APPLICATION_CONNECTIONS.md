...
应用验收应执行真实事务与读后写校验，不能只检查 SELECT 连通性。
...
`mysql_router_max_idle_server_connections` 默认值为 `0`：Router 不在不同客户端之间
复用空闲后端连接，自动读写分离仍可在同一客户端会话内管理读写连接。固定 RW / RO
端口保持原有会话语义；不通过全局启用 connection sharing 改变这些端口的应用行为。

应用侧仍应使用有界连接池，减少短连接引起的 TLS 握手和认证开销。若要调高 Router
空闲池上限，应先在目标 Router 版本验证三个入口交替连接、事务切换和会话状态隔离，
再通过 [滚动配置](../scenarios/CONFIGURATION.md) 应用。
