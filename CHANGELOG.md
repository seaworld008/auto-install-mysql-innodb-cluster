...
- 增加独立、共置、混合、三节点接入层的方案索引、拓扑模板与逐项操作文档。
- 主入口支持单独部署 HAProxy / Keepalived，以及只读检查的分层 scope。
- 支持显式 MySQL 成员通告地址，新集群可按 inventory IP 发现成员；已有地址受到变更保护。
- 提供四种本地模拟拓扑、原生内核 VM 配置、业务探针与写入账本校验，保留可重复执行的测试方案。

### 修复

- Router 默认关闭跨客户端空闲连接复用，保持混合 RW/RO/Split 事务路由与固定端口会话语义。
- MySQL / Router 按配置管理 systemd 文件句柄预算，检测冲突并修复运行进程限制漂移；表定义缓存遵守服务端最低值。
- 内核配置使用单一受管文件，移除失效参数，保护全局文件句柄上限，并实际核验 sysctl、THP、I/O 与冷启动持久化。
- 前置通告地址读取和滚动 ONLINE 等待使用已有 root 通道，支持实际主节点不在清单首位时重建管理员账号或收敛密码。
- 按操作校验所需凭据，扩容目标与备份执行组增加归属校验，保留多成员备份能力与最低 HA 门槛。
- XtraBackup 清单记录实际准备状态；已准备产物移除压缩残件，恢复按检查点选择步骤，避免重新解压覆盖数据页。
- 修复 Bash 3 空参数数组造成预检遗漏的问题；运行范围与凭据上下文在用户变量之后确定。
- 拒绝重复部署地址和入口优先级；内核备份与 sysctl 源目录分离，备份失败时停止修改。
- 测试工具拒绝覆盖已有环境、基线与账本；执行前后验证探针摘要，要求明确指定本轮预期账本。

### 验证

- 93 项单元/契约测试及静态、Ansible 语法、inventory 和文档检查通过。
- 实际完成四种拓扑、独立组件、滚动更新、故障、扩缩容及隔离备份恢复验证；完整记录见 [本机验证报告](docs/reports/LOCAL_SIMULATION_2026-09-10.md)。
- 本机功能结果不替代真实 RHEL 内核、安全策略、跨主机网络故障和生产容量验收。
